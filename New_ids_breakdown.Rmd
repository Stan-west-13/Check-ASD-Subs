...
new_ids <-read_xlsx("data/GUIDS_new_March_2025_aut_dx_confirmed 2.xlsx")
...
word_dict_S <- word_dict_S %>%
  left_join(cdi_metadata %>% select(num_item_id_cdi = num_item_id,word), by = c("CDI_Metadata_compatible" = "word")) %>%
  select(-num_item_id) %>%
  rename(num_item_id = num_item_id_cdi)


word_dict_WG <- word_dict_WG %>%
  left_join(cdi_metadata %>% select(num_item_id_cdi = num_item_id,word), by = c("CDI_Metadata_compatible" = "word")) %>%
  select(-num_item_id) %>%
  rename(num_item_id = num_item_id_cdi)
